...
if ($_POST)
	{
		...
		if (!empty ($_POST['jad_user']) && ($_POST['jad_pass']) && ($_POST['interface']) && ($_POST['jad_server']))
		{
			...
			$retval = 0;
			file_notice("ahool-ID",$error,"ahoolSquid - " . gettext($join), "");
			file_notice("ahool-ID",$error,"ahoolSquid - " . gettext($msg), "");
		}
		else
		{
...
$tab_array = array();
$tab_array[] = array(gettext("Remove Ahool from a DOMAIN"), false, "ad_remove.php");
$tab_array[] = array(gettext("Join Ahool to a DOMAIN"), true, "ad_join.php");
...
$section = new Form_Section('Join Ahool to DOMAIN');
